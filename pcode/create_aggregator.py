...
# import pcode.aggregation.lg_fedavg as lg_fedavg
...
# import pcode.aggregation.two_level_kd as two_level_kd
...
class Aggregator(object):
    ...
    def _define_aggregate_fn(self):
        ...
        elif self.conf.fl_aggregate["scheme"] == "lg_fedavg":
            self.aggregate_fn = self._s2_lg_fedavg()
        ...
        elif self.conf.fl_aggregate["scheme"] == "two_level_knowledge_transfer":
            self.aggregate_fn = self._s11_two_level_knowledge_transfer()
        else:
    ...
    def _s2_lg_fedavg(self):
        # local-global averaging scheme.
        def f(**kwargs):
            return lg_fedavg.lg_fedavg(
                conf=self.conf,
                clientid2arch=self.clientid2arch,
                n_selected_clients=kwargs["n_selected_clients"],
                flatten_local_models=kwargs["flatten_local_models"],
    ...
    def _s11_two_level_knowledge_transfer(self):
        def f(**kwargs):
            _client_models = two_level_kd.aggregate(
                conf=self.conf,
                fedavg_models=kwargs["fedavg_models"],
                client_models=kwargs["client_models"],
                criterion=self.criterion,
                ...
                # distillation_sampler=self.data_info["sampler"],
                # distillation_data_loader=self.data_info["data_loader"],
                distillation_data_loader=self.data_info["self_val_data_loader"],
                val_data_loader=self.data_info["self_val_data_loader"],
                # test_data_loader=self.test_loaders[0],
            )
            return _client_models
    ...
    def aggregate(
        ...
        **kwargs,
        ...
        # self.logger.log(f"local_train_scheme:{self.conf.local_train_scheme}; local_use_mutual:{self.conf.local_use_mutual}; local_attn_target_stu:{self.conf.local_attn_target_stu}")
        _aggregate_fn = (
            self.aggregate_fn
